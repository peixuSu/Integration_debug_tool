...
class SPIWindow(QObject):
    ...
    def __init__(self, application, driver=None):
        ...
        self.driver = driver
        self.log_window = LogWindow(application)
        ...
        self.device_detection_timer.start(500)  # 每500毫秒检测一次
...
#===============================================================================
    ...
    def vcc_changed(self, index):
        ...
        try:
            ...
            if result != 0:
                self.log_window.log(f"VCC设置失败,{self.driver.ERROR_CODES.get(result)}")
                return

            self.log_window.log(f"VCC设置为: {self.ui.combo_box_vcc.currentText()}")
            ...
            self.log_window.log(f"VCC设置失败: {str(e)}")
    ...
    def io_changed(self, index):
        ...
        try:
            ...
            if result != 0:
                self.log_window.log(f"IO电压设置失败,{self.driver.ERROR_CODES.get(result)}")
                return

            self.log_window.log(f"IO电压设置为: {self.ui.combo_box_io.currentText()}")
            ...
            self.log_window.log(f"IO电压设置失败: {str(e)}")
    ...
    def speed_changed(self, index):
        ...
        try:
            ...
            if result != 0:
                self.log_window.log(f"SPI速度设置失败,{self.driver.ERROR_CODES.get(result)}")
                return
            
            self.log_window.log(f"SPI速度设置为: {self.ui.combo_box_speed.currentText()}")
            ...
            self.log_window.log(f"SPI速度设置失败: {str(e)}")
    ...
    def clk_changed(self, index):
        ...
        self.log_window.log(f"SPI时钟模式设置为: {self.ui.combo_box_clk.currentText()}")
    ...
    def bit_changed(self, index):
        ...
        self.log_window.log(f"SPI位序设置为: {self.ui.combo_box_bit.currentText()}")
    ...
    def s_or_q_changed(self, index):
        ...
        self.log_window.log(f"SPI模式设置为: {self.ui.combo_box_s_or_q.currentText()}")
    ...
    def size_changed(self, index):
        ...
        self.log_window.log(f"接收数据字节数设置为: {self.ui.combo_box_size.currentText()}")
