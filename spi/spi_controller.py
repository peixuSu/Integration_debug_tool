...
class SPIController(QObject):
    ...
    def spi_send(self, data, clk_mode, bit_order, log = True, data_name = ""):
        ...
        if self.ui.checkBox_self_test.isChecked():
            ...
            if result == 0:
                if log is True and data_name != "":
                    self.log_signal.emit(f"发送成功,{data_name},{' '.join([f'{byte:02X}' for byte in data_bytes])}")
                elif log is True:
                    self.log_signal.emit(f"发送成功: {' '.join([f'{byte:02X}' for byte in data_bytes])}")
            else:
                if log is True:
                    self.log_signal.emit(f"数据发送失败: {self.driver.ERROR_CODES.get(result)}")
    ...
    def spi_receive(self, clk_mode, bit_order, rx_size, log = True):
        ...
        if result == 0:
            ...
            if log is True:
                self.log_signal.emit(f"数据接收: {formatted_data}")

            return rx_buffer
        else:
            if log is True:
                self.log_signal.emit(f"数据接收失败: {self.driver.ERROR_CODES.get(result)}")
    ...
    def spi_transfer(self, data, clk_mode, bit_order, log = True):
        ...
        if result == 0:
            if log is True:
                ...
                self.log_signal.emit(f"发送: {' '.join([f'{byte:02X}' for byte in data_bytes])}, 接收: {formatted_data}")
        else:
            if log is True:
                self.log_signal.emit(f"SPI全双工传输异常: {self.driver.ERROR_CODES.get(result)}")